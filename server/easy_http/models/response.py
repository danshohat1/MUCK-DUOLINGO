...
class Response(Request, Send):
    def __init__(self, app: App, client_socket: socket, addr: Tuple[str, int],
                 cors: Cors):
    ...
    def handle_http_request(self):
        ...
        if self.details["method"].value == "OPTIONS":
            ...
            self.send(client_socket=self.client_socket, msg=response.data,
                      status=response.status.value, cookies=response.cookies,
                      cors=self.cors)
            return

        route = list(filter(
            lambda current_route: current_route.path == self.details[
                "path"] and current_route.method == self.details[
                                      "method"],
            self.app.routes.keys()))
        ...
        if route.authorization:
            key = [val for key, val in self.details["data"].items() if
                   key.lower() == AUTHORIZATION_REQUEST_PARAM]

            if not key:
                print(ExceptionPrompt(
                    "Route with authorization must "
                    "have an authorization key in the params",
                    ExceptionTypes.RUNTIME))
                sys.exit()
    ...
    def send_prompt(self, response: Union[ResponseScheme, Render]) -> None:
        ...
        self.send(client_socket=self.client_socket, msg=response.data,
                  ...
                  cors=self.cors)
        print(
            ResponsePrompt(response.status,
                           self.details["method"], self.origin,
                           self.details["path"]))
    ...
    def call_route(self, route):
        try:
            ...
            response = route(*(
                    self.details["query_params"] + [value for key, value in
                                                    self.details[
                                                        "data"].items() if
                                                    key.lower() !=
                                                    AUTHORIZATION_REQUEST_PARAM
                                                    ]))
            self.send_prompt(response)
        except Exception as e:
            print(e)
            warnings.warn(
                f"{route.__name__} should return a ResponseScheme type",
                RuntimeWarning)
