...
class Request:
    def __init__(self, client_socket: socket.socket) -> None:
        """Initialize the HTTP handler with the client socket, receive and
         decode the client's request, and validate it."""
    ...
    def generate_friendly_request(self) -> Dict[str, Any]:
        """Split the raw HTTP request into a list and generate a
         user-friendly request with details."""
        ret = self.__client_request.split("\r\n")

        # get cookies
        cookies = list(filter(lambda header: header.startswith("Cookie: "),
                              self.__client_request.split("\r\n")))
        if not cookies:
            cookies = [None]

        return self.generate_friendly_details(
            ret[0].split() + [ret[-1]] + cookies)
    ...
    def generate_friendly_details(self, details: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "path": details[1] if details[1].find("?") == -1 else
            details[1].split("?")[0],
            "version": details[2],
    ...
    def extract_query_params(self, path: str) -> List[str]:
        ...
        if query_start_index != -1:
            ...
            query_params = [unquote(param.split("=", 1)[1].strip())
                            for param in
                            query_string.split("&") if
                            "=" in param]
        return query_params
