...
class Authorization:
    def __init__(self, true_case: Callable = lambda arg: None,
                 false_case: Callable = lambda: None) -> None:

        if len(inspect.signature(true_case).parameters) != 1:
            raise TypeError(
                f"The true_case function must have "
                f"one parameter (dict type)." +
                f" got {len(inspect.signature(true_case).parameters)}")

        if len(inspect.signature(false_case).parameters) != 0:
            raise TypeError(
                f"The false_case function must have no parameters." +
                f" got {len(inspect.signature(true_case).parameters)}")
    ...
    def check_authorization(self, key) -> Optional[ResponseScheme]:
        try:
            decoded_token = jwt.decode(str(key), self.secret_key,
                                       algorithms=["HS256"])
            ...
            return self.true_res(dict(
                [(key, val) for key, val in decoded_token.items() if
                 key != "exp"]))
