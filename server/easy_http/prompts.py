...
class Prompt:
    ...
    def __str__(self) -> str:
        if isinstance(self, (WarningPrompt, ExceptionPrompt)) or not \
                self.inclue_date:
            return self.content
        if isinstance(self, ResponsePrompt):
            content = self.colored_prompt(prompt_text=self.content,
                                          color=Fore.CYAN) \
                if self.status == Statuses.OK else self.colored_prompt(
                prompt_text=self.content, color=Fore.RED)
...
class WarningPrompt(Prompt):
    def __init__(self, content: str, type: WarningTypes) -> None:
        content = self.colored_prompt(f"{type.value} Warning: ",
                                      Fore.YELLOW) + content
...
class ResponsePrompt(Prompt):
    ...
    def __init__(self,
                 status: Statuses, method: HttpMethod, ip: str, path: str,
                 version: str = "HTTP/1.1") -> None:
...
class ExceptionPrompt(Prompt):
    def __init__(self, exception_message: str, type: ExceptionTypes) -> None:
        content = self.colored_prompt(f"{type.value} Exception: ",
                                      Fore.RED) + exception_message
        self.type = type
        super().__init__(content)
