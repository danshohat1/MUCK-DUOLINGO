...
class FillSentenceQuestion:
    ...
    def __init__(self, sentence: str, answer: str, words: List[Dict[str,
                                                                    str]],
                 number_of_options=8):
        self.words = words
    ...
    def get_block(self, term: str) -> Dict[str, str]:
        return \
                list(filter(lambda word: list(word.keys())[0] == term,
                            self.words))[0]
    ...
    def generate_blocks(self) -> List[Dict[str, str]]:
        ...
        print("muck1")
        answer_words = list(
            map(lambda term: self.get_block(term.replace("_", " ")),
                answer_words))
        print("muck2")
        print(answer_words)
        ...
        wrong_words = [word for i, word in enumerate(self.words) if
                       word not in answer_words and i < self.number_of_options]
