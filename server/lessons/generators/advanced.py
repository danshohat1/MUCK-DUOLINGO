...
class Advanced(Generator):
    def __init__(self, questions: List[Dict[Any, Any]],
    ...
    def generate_question(self, mold: Dict[str, Any]) -> Question:
        ...
        if question is AdvancedMultipleChoice or \
                question is FillSentenceQuestion:
            # return the multiple choice obj
            return {**question_type, **dict(question(**dict(
                [(key, val) for key, val in mold.items() if key != "type"]),
                                                     words=self._words))}
        return {**question_type, **dict(question(**dict(
            [(key, val) for key, val in mold.items() if key != "type"])))}
